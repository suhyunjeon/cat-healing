(() => {
  ...
  const items = [
    ...
    {id:'loveBrush',name:'고급 사랑빗',cost:120,left:'32%',bottom:'24%',art:'brush',description:'하트 손잡이의 다정한 원목 빗'},
    {id:'blanket',name:'포근한 체크 담요',cost:160,left:'18%',bottom:'38%',art:'blanket',description:'크림빛 양털 테두리와 세이지 체크'},
    {id:'scratcher',name:'라운지 스크래쳐',cost:220,left:'60%',bottom:'22%',art:'scratcher',description:'쭉 기지개 켜고 쉬어가는 곡선 쉼터'},
    {id:'carrier',name:'프리미엄 이동장',cost:300,left:'9%',bottom:'23%',art:'carrier',description:'둥근 크림색 몸체와 세이지 손잡이'},
  ...
  window.addEventListener('cat-adventure-reward',event=>{const reward=event.detail;if(!reward||!Number.isFinite(reward.xp))return;freshDay();progress.xp+=Math.max(0,Math.min(15,reward.xp));if(reward.play)progress.counts.play++;render()});
